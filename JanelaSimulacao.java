        ...
        for(int i = 0; i < mapa.getAltura(); i++){
            for(int j = 0; j < mapa.getLargura(); j++){
                if(mapa.getItem(i, j) != null){//Se existir algum objeto na posicao (i,j)
                    EntidadesCenario entidade = mapa.getItem(i, j);
                    Localizacao localizacao = entidade.getLocalizacaoAtual();
                    visaoMapa.desenharImagem(localizacao.getX(), localizacao.getY(), entidade.getImagem());
                }
            }
        }
        visaoMapa.repaint();
